...
switch($modx->event->name){
    ...
    case 'OnPageNotFound':{
        if(class_exists('Twig_Autoloader')){
            ...
            $modx->cache = new Doctrine\Common\Cache\FilesystemCache( $cachePath );
            ...
            */
            ...
            $modx->cache = $modx->twig = new xNop;
        }
        $modx->useConditional = $conditional && !$debug;
        break;
    }
    case 'OnAfterLoadDocumentObject':{
        //if(file_exists(MODX_BASE_PATH.'assets/element/template/'.$documentObject['template'].'.html')){
        switch(true){
            case file_exists(MODX_BASE_PATH.'assets/element/template/'.'tpl-'.$documentObject['template'].'_doc-'.$documentObject['id'].'.twig.html'):{
            ...
            case file_exists(MODX_BASE_PATH.'assets/element/template/doc-'.$documentObject['id'].'.twig.html'):{
                $modx->docTemplate = 'doc-'.$documentObject['id'];
                $documentObject['oldTemplate'] = $documentObject['template'];
                $documentObject['template'] = 0;
                break;
            }
            case file_exists(MODX_BASE_PATH.'assets/element/template/tpl-'.$documentObject['template'].'.twig.html'):{
